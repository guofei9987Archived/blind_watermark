...
def anti_cut_att(input_filename, output_file_name, origin_shape):
    ...
    if input_img_shape[0] > origin_shape[0] or input_img_shape[0] > origin_shape[0]:
        print('裁剪打击后的图片，不可能比原始图片大，检查一下')
        return

    # 还原纵向打击
    if input_img_shape[0] < origin_shape[0]:
        output_img = np.concatenate([output_img, output_img[:origin_shape[0] - input_img_shape[0], :, :]], axis=0)
    if input_img_shape[1] < origin_shape[1]:
        output_img = np.concatenate([output_img, output_img[:, :origin_shape[1] - input_img_shape[1], :]], axis=1)
